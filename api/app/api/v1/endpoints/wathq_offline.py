...
def get_my_offline_data(
    ...
    if isinstance(current_user, models.ManagementUser):
        ...
        offline_data = (
            ...
            .filter(models.WathqOfflineData.management_user_id == current_user.id)
            .order_by(models.WathqOfflineData.fetched_at.desc())
            .offset(skip)
            .limit(limit)
            .all()
        )
...
@router.get(
    "/service/{service_identifier}", response_model=List[schemas.WathqOfflineData]
)
def get_offline_data_by_service(
    ...
    try:
        ...
        service = (
            ...
            .filter(models.Service.slug == service_identifier)
            .first()
        )
        if not service:
...
@router.get("/{identifier}", response_model=List[schemas.WathqOfflineData])
def get_offline_data_by_identifier(
    ...
    """
    Get offline WATHQ data by flexible identifier.
    identifier can be either a UUID (for specific record) or a service slug (e.g., 'commercial-registration').
    - If UUID: returns single record if found
    - If service slug: returns list of records for that service
    Works for both tenant users and management users.
    """

    # Try to parse as UUID first
    try:
        data_id = UUID(identifier)
        # It's a UUID, get specific record
        if isinstance(current_user, models.ManagementUser):
            offline_data = (
                db.query(models.WathqOfflineData)
                .filter(
        ...
        return [offline_data]  # Return as list for consistent response format

    except ValueError:
        # Not a UUID, treat as service slug
        service = (
            db.query(models.Service).filter(models.Service.slug == identifier).first()
        )
        if not service:
            raise HTTPException(status_code=404, detail="Service not found")
