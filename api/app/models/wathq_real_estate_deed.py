...
class Deed(Base):
    ...
    Same deed data can be repeated but each record is linked to a unique call log.
    """
    ...
    # Relationships
    owners = relationship(
        "DeedOwner", back_populates="deed", cascade="all, delete-orphan"
    )
    real_estates = relationship(
        "DeedRealEstate", back_populates="deed", cascade="all, delete-orphan"
    )
...
class DeedOwner(Base):
    ...
    id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    deed_id = Column(
        Integer, ForeignKey("wathq.deeds.id", ondelete="CASCADE"), nullable=False
    )
...
class DeedRealEstate(Base):
    ...
    border_east_length = Column(
        String(50), nullable=True
    )  # VARCHAR due to mixed input like '22ز6'
