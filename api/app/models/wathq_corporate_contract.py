...
from sqlalchemy import Column, Integer, String, Boolean, Date, Numeric, Text, DateTime, JSON, ForeignKey, func
...
class CorporateContract(Base):
    ...
    # Relationships
    stocks = relationship("ContractStock", back_populates="contract", cascade="all, delete-orphan")
    parties = relationship("ContractParty", back_populates="contract", cascade="all, delete-orphan")
    managers = relationship("ContractManager", back_populates="contract", cascade="all, delete-orphan")
    management_config = relationship("ContractManagementConfig", back_populates="contract", cascade="all, delete-orphan", uselist=False)
    activities = relationship("ContractActivity", back_populates="contract", cascade="all, delete-orphan")
    articles = relationship("ContractArticle", back_populates="contract", cascade="all, delete-orphan")
    decisions = relationship("ContractDecision", back_populates="contract", cascade="all, delete-orphan")
    notification_channels = relationship("NotificationChannel", back_populates="contract", cascade="all, delete-orphan")
...
class ContractStock(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    contract_id = Column(Integer, ForeignKey('wathq.corporate_contracts.id', ondelete='CASCADE'), nullable=False)
